...
while i < len(lines) - 1:
    ...
    if line.find("int cbf_") >= 0:  # We found a function
        ...
        check = 0
        while (
            lines[i + 1].find("DESCRIPTION") == -1
            and lines[i + 1].find("int cbf_") == -1
        ):
            i = i + 1
    ...
    if on == 1:
        ...
        if (
            line.strip()[0] in [str(j) for j in range(9)]
            or line.find("SEE ALSO") >= 0
            or line.find("________") >= 0
            or line.find("--------") >= 0
        ):
            if len(docstring) > 0:
...
""",
        ...
        [],
        ["int compression", "int binary_id", "int elsize", "int elements"],
    ],
...
""",
        "get_real_image_as_string",
        ["int element_number", "int elsize", "int ndimslow", "int ndimfast"],
        ["(Binary)String"],
    ],
...
""",
        "get_real_image_fs_as_string",
        ["int element_number", "int elsize", "int ndimfast", "int ndimslow"],
        ["(Binary)String"],
    ],
...
""",
        "get_pixel_size",
        ["Int element_number", "Int axis_number"],
        ["Float pixel_size"],
    ],
...
""",
        "write_file",
        ["String filename", "Integer ciforcbf", "Integer Headers", "Integer encoding"],
        [],
    ],
    "cbf_write_widefile": [
        """
...
""",
        ...
        ["String Columnvalue", "Int default"],
        ["Int Value"],
    ],
    "cbf_require_value": [
        """
...
""",
        "set_axis_setting",
        ["String axis_id", "Float start", "Float increment"],
        [],
    ],
...
""",
        "get_axis_ancestor",
        ["String axis_id", "Integer ancestor_index"],
        ["String"],
    ],
...
""",
        "set_timestamp",
        ["Float time", "Integer timezone", "Float precision"],
        [],
    ],
...
""",
        "set_overload",
        ["Integer element_number", "Float overload"],
        [],
    ],
...
""",
        "set_tag_root",
        ["String tagname", "String tagroot_in"],
        [],
    ],
...
""",
        "set_category_root",
        ["String categoryname", "String categoryroot"],
        [],
    ],
...
""",
        "set_bin_sizes",
        ["Integer element_number", "Float slowbinsize_in", "Float fastbinsize_in"],
        [],
    ],
    "cbf_get_bin_sizes": [
        """
...
""",
        "get_bin_sizes",
        ["Integer element_number"],
        ["Float slowbinsize", "Float fastbinsize"],
    ],
    ...
    def wrap(self, cfunc, prototype, args, docstring):
        ...
        if len(args) == 2:
            if (
                ...
                return
            if (
                args[1].find("const char") > -1 and args[1].find("**") > -1
            ):  # return string
                code += docstringwrite(pyfunc, ["string"], [], prototype, docstring)
            ...
            if args[1].find("unsigned int") > -1 and args[1].find("*") == -1:
                ...
                if args[1].find("reserved") > -1:
                    raise Exception(
                        "Setting reserved??? %s %s %s" % (pyfunc, cfunc, str(args))
                    )
            ...
            if args[1].find("cbf") == -1:  # but do not try the goniometer constructor
                if args[1].find("*") > -1 and args[1].find("cbf") == -1:
                    ...
                    type = args[1].split(" ")[0]
                    code += docstringwrite(
                        pyfunc, [], [type.replace("*", "")], prototype, docstring
                    )
...
""",
        "get_pixel_normal_fs",
        ["double indexfast", "double indexslow"],
        ["double normal1", "double normal2", "double normal3"],
    ],
...
""",
        "get_pixel_normal_sf",
        ["double indexslow", "double indexfast"],
        ["double normal1", "double normal2", "double normal3"],
    ],
...
"""
    ...
    def wrap(self, cfunc, prototype, args, docstring):
        ...
        try:
            ...
            raise
        if (
            len(args) == 1 and args[0].find("char") > -1 and args[0].find("**") > -1
        ):  # return string
    ...
    for cname in sorted(names):
        ...
        if args[0].find("cbf_handle") >= 0:  # This is for the cbfhandle object
            ...
            if cname == "cbf_get_unit_cell":
                cbf_handle_wrapper.wrap(
                    "cbf_get_unit_cell_esd", prototype, args, docstring
                )
            if cname == "cbf_get_reciprocal_cell":
                cbf_handle_wrapper.wrap(
                    "cbf_get_reciprocal_cell_esd", prototype, args, docstring
                )
            if cname == "cbf_set_unit_cell":
                cbf_handle_wrapper.wrap(
                    "cbf_set_unit_cell_esd", prototype, args, docstring
                )
            if cname == "cbf_set_reciprocal_cell":
                cbf_handle_wrapper.wrap(
                    "cbf_set_reciprocal_cell_esd", prototype, args, docstring
                )
            continue
