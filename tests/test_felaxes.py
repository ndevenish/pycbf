...
def test_felaxes():
    ...
    for element in range(64):
        ...
        print(
            "(0, 0) v00 [ %.9f %.9f %.9f ]"
            % (round(v00[0], 9), round(v00[1], 9), round(v00[2], 9))
        )
        print(
            "(0, 1) v01 [ %.9g %.9g %.9g ]"
            % (round(v01[0], 9), round(v01[1], 9), round(v01[2], 9))
        )
        print(
            "(1, 0) v10 [ %.9g %.9g %.9g ]"
            % (round(v10[0], 9), round(v10[1], 9), round(v10[2], 9))
        )
        print(
            "(1, 1) v11 [ %.9g %.9g %.9g ]"
            % (round(v11[0], 9), round(v11[1], 9), round(v11[2], 9))
        )

        print(
            "surface axes:",
            d.get_detector_surface_axes(0),
            d.get_detector_surface_axes(1),
        )

        print(
            d.get_detector_surface_axes(0),
            "has",
            cbf.count_axis_ancestors(d.get_detector_surface_axes(0)),
            "ancestors",
        )
        print(
            d.get_detector_surface_axes(1),
            "has",
            cbf.count_axis_ancestors(d.get_detector_surface_axes(1)),
            "ancestors",
        )
        ...
        for index in range(count):
            ...
            print(
                "    vector [ %.8g %.8g %.8g ]"
                % (round(vector[0], 7), round(vector[1], 7), round(vector[2], 7))
            )
            offset = cbf.get_axis_offset(cur_axis)
            print(
                "    offset [ %.8g %.8g %.8g ]"
                % (round(offset[0], 7), round(offset[1], 7), round(offset[2], 7))
            )
